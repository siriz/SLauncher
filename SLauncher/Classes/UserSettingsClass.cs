...
namespace SLauncher.Classes
{
    ...
    public static class UserSettingsClass
    {
        ...
        // System Tray Settings
        /// <summary>
        /// Variable which stores whether to minimize to tray instead of closing
        /// </summary>
        ...
        /// <summary>
        /// Variable which stores the global hotkey combination
        /// </summary>
   ...
   public static bool AlwaysOnTop = false;
        ...
        /// <summary>
    /// Variable which stores the preferred language code (e.g., "en-US", "ko-KR", "ja-JP")
 /// </summary>
        public static string Language = "";

        // DIRECTORIES
        /// <summary>
        /// Root directory where user data for LauncherX is stored (in executable folder)
        /// </summary>
        public static string SettingsDir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "UserCache");

        /// <summary>
        /// Directory where the items added to LauncherX are stored
        /// </summary>
        public static string DataDir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "UserCache", "Files");
 ...
 GridPosition = userSettingsJson.gridPosition;
  MinimizeToTray = userSettingsJson.minimizeToTray;
  StartWithWindows = userSettingsJson.startWithWindows;
   GlobalHotkey = userSettingsJson.globalHotkey;
       AlwaysOnTop = userSettingsJson.alwaysOnTop;
                Language = userSettingsJson.language ?? "";
    }
    }
...
System.Diagnostics.Debug.WriteLine($"  Saved group {globalFilename}: {gridViewTileGroup.DisplayText} ({gridViewTileGroup.Items.Count} items)");
  ...
  /// </summary>
        /// <param name="tabs">List of TabViewItem objects</param>
        /// <param name="allItems">All items that should be indexed</param>
   /// <param name="tabColors">Dictionary of tab colors</param>
        /// <param name="selectedTabIndex">Currently selected tab index</param>
  ...
  Dictionary<Microsoft.UI.Xaml.Controls.TabViewItem, Windows.UI.Color> tabColors,
      ...
      {
  // Convert WinRT collection to standard list
     var tabsList = new List<Microsoft.UI.Xaml.Controls.TabViewItem>();
   for (int i = 0; i < tabs.Count; i++)
 {
     if (tabs[i] is Microsoft.UI.Xaml.Controls.TabViewItem tab)
          {
          tabsList.Add(tab);
     }
  }
 ...
 SaveTabsWithItemList(tabsList, itemsList, tabColors, selectedTabIndex);
 }
  ...
  List<Microsoft.UI.Xaml.Controls.TabViewItem> tabs,  // ? Changed from IList
  ...
  for (int tabIndex = 0; tabIndex < tabs.Count; tabIndex++)
   {
  var tab = tabs[tabIndex];  // ? Direct access, no casting needed
...
int itemIndex = allItems.IndexOf(item);
 ...
 System.Diagnostics.Debug.WriteLine($"Successfully wrote tabs.json to {tabsFilePath}");
  ...
  if (!File.Exists(tabsFilePath))
    {
        System.Diagnostics.Debug.WriteLine("tabs.json not found");
   return null;
         }
      ...
      string jsonString = File.ReadAllText(tabsFilePath);
       System.Diagnostics.Debug.WriteLine($"DEBUG LoadTabs: Read tabs.json:\n{jsonString}");
                ...
                var tabsData = JsonSerializer.Deserialize<TabsData>(jsonString, SourceGenerationContext.Default.TabsData);
 System.Diagnostics.Debug.WriteLine($"DEBUG LoadTabs: Loaded {tabsData?.Tabs.Count ?? 0} tabs");
     ...
     {
         System.Diagnostics.Debug.WriteLine($"ERROR LoadTabs: {ex.Message}\n{ex.StackTrace}");
        return null;
 }
        }
 ...
 /// <summary>
        ...
        /// </summary>
  /// <param name="allItems">All loaded items</param>
/// <param name="tabsData">Saved tabs data</param>
        /// <returns>Dictionary mapping tab IDs to their items</returns>
...
string cleanIndexStr = indexStr.TrimEnd('/');
       ...
       if (itemIndex >= 0 && itemIndex < allItems.Count)
   {
    ...
    System.Diagnostics.Debug.WriteLine($"Distributed {tabItems.Count} items to tab '{tabData.Name}'");
  }
